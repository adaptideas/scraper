...
final public class ScraperSpecificationTest {
    ...
    public void testThatTagWithAttributesWorksWithCaptureGroup() {
        List<Item> items = new Template<Item>("<td class=\"test\">${test}\n</td>", Item.class).match(new Html(
                "<td>content</td>"));
        Assert.assertEquals(0, items.size());

        items = new Template<Item>("<td class=\"test\">\n${test}</td>", Item.class).match(new Html(
                "<td class=\"test\">content</td><td>should not capture</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("content", items.get(0).test());

        items = new Template<Item>("<td class=\"test\">${test}\n</td>", Item.class).match(new Html(
                "<td class=\"test\">content</td><td>should not capture</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("content", items.get(0).test());
    }
    ...
    public void testThatTagWithContentWorksWithCaptureGroup() {
        List<Item> items = new Template<Item>("<td>content</td>\n${test}", Item.class).match(new Html("<td></td>test"));
        Assert.assertEquals(0, items.size());

        items = new Template<Item>("<td>content</td>\n${test}</td>", Item.class).match(new Html(
                "<td>content</td><td>should capture</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("should capture", items.get(0).test());

        items = new Template<Item>("<td>content</td>${test}\n</td>", Item.class).match(new Html(
                "<td>content</td><td>should capture</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("should capture", items.get(0).test());
    }
    ...
    public void testThatTagWithContentWorksWithCaptureGroupAndContent() {
        List<Item> items = new Template<Item>("<td>content ${test}</td>", Item.class).match(new Html(
                "<td>cont capture</td>"));
        Assert.assertEquals(0, items.size());

        items = new Template<Item>("<td>content ${test}</td>", Item.class).match(new Html("<td>content capture</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("capture", items.get(0).test());

        items = new Template<Item>("<td>${test} content</td>", Item.class).match(new Html("<td>capture content</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("capture", items.get(0).test());
    }
    ...
    public void testThatTagWithContentWorksWithMultipleCaptureGroupsAndContent() {
        List<Item> items = new Template<Item>("<td>${foo} content ${test}</td>", Item.class).match(new Html(
                "<td>bar cont capture</td>"));
        Assert.assertEquals(0, items.size());

        items = new Template<Item>("<td>${foo} content ${test}</td>", Item.class).match(new Html(
                "<td>bar content capture</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("capture", items.get(0).test());
        Assert.assertEquals("bar", items.get(0).foo());

        items = new Template<Item>("<td>ignore ${foo} content ${test}</td>", Item.class).match(new Html(
                "<td>ignore bar content capture</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("capture", items.get(0).test());
        Assert.assertEquals("bar", items.get(0).foo());

        items = new Template<Item>("<td>ignore ${foo} content ${test} ignore</td>", Item.class).match(new Html(
    ...
    public void testThatTagWorksWithCaptureGroupsAndIgnoreAllMatcher() {
        List<Item> items = new Template<Item>("<td>${test} hour ...</td>", Item.class).match(new Html(
                "<td>capture hour just some text to ignore</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("capture", items.get(0).test());

        items = new Template<Item>("<td>before ${test} hour ...</td>", Item.class).match(new Html(
    ...
    public void testThatTagWorksWithMultipleCaptureGroupsAndIgnoreAllMatcher() {
        List<Item> items = new Template<Item>("<td>${foo} ignore ${test} hour ...</td>", Item.class).match(new Html(
                "<td>something ignore capture hour just some text to ignore</td>"));
        Assert.assertEquals(1, items.size());
        Assert.assertEquals("capture", items.get(0).test());
        Assert.assertEquals("something", items.get(0).foo());

        items = new Template<Item>("<td>before ${foo} before ${test} hour ...</td>", Item.class).match(new Html(
